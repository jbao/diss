...
%Then, for each pathway, the number of genes that are part of the 
%pathway is counted. 
...
%\caption[H838 experimental setup]{\small{
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=\textwidth]{h838-gage_geneset_basal.pdf}
\end{center}
...
In order to understand how the transcriptional program
...
%By grouping the time points into 3 
...
%crosslinking with other signaling pathways.
%On the other hand, a number of different pathways 
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=\textwidth]{h838-mfpt_sp_z_qqplot.pdf}
\end{center}
...
We next set out to compare both shortest path length (SP) and mean first 
...
\hskip 0.5in A \hskip 2.5in B
\begin{center}
\includegraphics[width=0.45\textwidth]{h838-mfpt_ranking.pdf}
\includegraphics[width=0.45\textwidth]{h838-mfpt_top_ts.pdf}
\end{center}
...
As a result, we ranked all receptors that are significant
...
\subsubsection{Supernatant  from different cocultures show distinct cytokine composition}
...
Based on the cytokine array results and the receptor inference from MFPT, 
...
%and GRO-$\alpha$ as a ligand for CXCR2, which is also highly expressed in the medium.
...
%The fact that \sdfonea
%is only present in the tumor-endothelial cell heterogeneous coculture conditioned medium makes it
%also a candidate in the migration determination, which is also 
...
%Since both receptors of
%\tnfa and
%\sdfonea are most strongly linked to the transcription
%factors shortly after the scratch in H838 cells, we hypothesize
%that the transcriptional program is switched on and thus the
%cellular decisions are made early in the process.

%These results are in line with the hypothesis 
...
%The sustained 
%close link
%between most receptors and transcription factors from 2 to 4 hours (\ref{fig:h838_mfpt_ts})
